...
def track_photos(matrix_coefficients, distortion_coefficients, dirpath="test_photos", image_format="jpg"):
    ...
    for fname in images:
        ...
        # operations on the frame come here
        if SIMPLE_POSE_ALGORITHM:
            frame, tvec = pose_estimation_simple(
                frame, matrix_coefficients, distortion_coefficients)
        else:
            frame, tvec = pose_estimation(
                frame, matrix_coefficients, distortion_coefficients)
    ...
    if np.all(ids is not None):  # If there are markers found by detector
        for i in range(0, len(ids)):  # Iterate in markers
            ...
            rvec, tvec, _ = cv2.aruco.estimatePoseSingleMarkers(
                corners, MARKER_LENGTH, matrix_coefficients, distortion_coefficients)
            ...
            # font
            frame = writeTextToFrame(frame, "%.1f,%.1f,%.1f" % (
                tvec[0][0][0], tvec[0][0][1], tvec[0][0][2]))
            frame = cv2.putText(frame, "%.2f,%.2f,%.2f" % (
...
if __name__ == "__main__":
    camera_matrix, dist_matrix = calibration.load_coefficients(
        "calibration/cameraCoeffs.yml")
    if(LIVE_VIDEO_MODE):
